...
export async function getItemsByWarehouse(warehouseId) {
  ...
  const res = await fetch(`${API_URL}/item-warehouses?id_almoxarifado=${warehouseId}`, {
    headers: { Authorization: `Bearer ${token}` },
  });

  const data = await res.json();
  if (!res.ok) throw new Error(data.error || "Erro ao buscar itens do almoxarifado");
  return data;
}
...
export async function getAllItemWarehouses() {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || "Erro ao buscar vínculos item-almoxarifado");
  return data;
}
...
export async function createItemWarehouse(itemWarehouse) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || "Erro ao criar entrada de estoque");
  return data;
}
